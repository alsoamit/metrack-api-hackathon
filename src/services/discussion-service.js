...
class DiscussionService {
  async getAll(filter) {
    try {
      const discussion = await DiscussionModel.find((filter = null))
        .sort("date")
        .limit(100)
        .populate("chat");
      return discussion;
  ...
  async update(filter, data) {
    return await DiscussionModel.updateOne(filter, data, { new: true });
  }

  async deleteOne(filter) {
    return await DiscussionModel.deleteOne(filter);
  }
}
